...
function isBrowserRequest(value) {
  ...
  return value.protocol_version === PROTOCOL_VERSION && isId(value.request_id) && isId(value.run_id) && Number.isSafeInteger(value.tab_id) && value.tab_id >= 0 && Number.isSafeInteger(value.timeout_ms) && value.timeout_ms >= 100 && value.timeout_ms <= 12e4 && isBrowserAction(value.action);
}
...
function isRunEvent(value) {
  return isRecordWithKeys(value, ["protocol_version", "type", "run_id", "event", "data"]) && value.protocol_version === PROTOCOL_VERSION && value.type === "run.event" && isId(value.run_id) && isId(value.event);
}
...
var RelayClient = class {
  ...
  async request(method, params, timeoutMs = 15e3) {
    ...
    const request = { protocol_version: PROTOCOL_VERSION, type: "control.request", request_id, method, params };
...
chrome.tabs.onRemoved.addListener((tabId) => {
  if (tabs.has(tabId)) {
    ...
    relay.send({ event: "tab_revoked", protocol_version: PROTOCOL_VERSION, tab_id: tabId });
  }
});
chrome.tabs.onUpdated.addListener((tabId, changeInfo) => {
  if (tabs.has(tabId) && changeInfo.groupId !== void 0) {
...
async function handleUiMessage(message) {
  ...
  switch (item.type) {
    ...
    case "tab.toggle": {
      ...
      return { ok: true, shared: await tabs.toggle(item.tabId) };
    }
    ...
    case "workflow.start":
      return { ok: true, result: await relay.request("workflow.start", { workflow_id: item.workflowId, tab_id: item.tabId }) };
